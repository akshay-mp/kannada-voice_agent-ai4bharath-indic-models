<script lang="ts">
  ...
  let bars = $derived.by(() => {
    ...
    else if (data.trans2EndTs) endTime = Math.max(endTime, data.trans2EndTs);
    else if (data.agentEndTs) endTime = Math.max(endTime, data.agentEndTs);
    else if (data.trans1EndTs) endTime = Math.max(endTime, data.trans1EndTs);
    ...
    return {
      ...
      trans1: {
        style: getBarStyle(baseTime, totalDuration, data.trans1StartTs, data.trans1EndTs, isActive && !!data.trans1StartTs && !data.trans1EndTs),
        duration: getDuration(data.trans1StartTs, data.trans1EndTs, isActive && !!data.trans1StartTs && !data.trans1EndTs),
      },
      agent: {
      ...
      trans2: {
        style: getBarStyle(baseTime, totalDuration, data.trans2StartTs, data.trans2EndTs, isActive && !!data.trans2StartTs && !data.trans2EndTs),
        duration: getDuration(data.trans2StartTs, data.trans2EndTs, isActive && !!data.trans2StartTs && !data.trans2EndTs),
      },
      tts: {
...
<div class="mt-5 pt-5 border-t border-gray-700">
  ...
  <div class="mb-4">
    {#if bars}
      {#each [
        { label: 'STT', bar: bars.stt, gradient: 'from-cyan-400 to-cyan-500' },
        { label: 'In→En', bar: bars.trans1, gradient: 'from-indigo-400 to-indigo-600' },
        { label: 'Agent', bar: bars.agent, gradient: 'from-purple-500 to-violet-600' },
        { label: 'En→In', bar: bars.trans2, gradient: 'from-emerald-400 to-emerald-600' },
