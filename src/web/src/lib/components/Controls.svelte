<script lang="ts">
  import { session, currentTurn } from '../stores';
  ...
  const statusConfig = {
    ...
    thinking: { dot: 'bg-purple-500 animate-pulse', text: 'Thinking...' },
    speaking: { dot: 'bg-orange-500 shadow-[0_0_8px_theme(colors.orange.500)] animate-pulse', text: 'Speaking...' },
    error: { dot: 'bg-red-500', text: 'Error' },
    disconnected: { dot: 'bg-gray-400', text: 'Disconnected' },
  };

  let config = $derived.by(() => {
    if ($session.status === 'listening' && $currentTurn.active) {
      if ($currentTurn.ttsStartTs) return statusConfig.speaking;
      if ($currentTurn.agentStartTs) return statusConfig.thinking;
      if ($currentTurn.sttEndTs) return statusConfig.processing;
    }
    return statusConfig[$session.status] || statusConfig.ready;
  });
</script>
